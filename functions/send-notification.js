...
export async function onRequestPost(context) {
    ...
    try {
        ...
        if (!key || !message) { return new Response(JSON.stringify({ error: "Missing data" }), { status: 400, headers: corsHeaders }); }
        if (!env.FIREBASE_SERVICE_ACCOUNT) { return new Response(JSON.stringify({ error: "Missing Firebase Credentials" }), { status: 500, headers: corsHeaders }); }
        ...
        if (!tokensObj) { return new Response(JSON.stringify({ success: true, message: "No tokens found" }), { headers: corsHeaders }); }
        ...
        for (const token of tokens) {
            ...
            const fcmPayload = {
                message: {
                    token: token,
                    notification: { title: type === 'new_session' ? `New Chat Started (Key: ${key})` : `New Message (Key: ${key})`, body: message },
                    webpush: { fcm_options: { link: `https://${request.headers.get("host")}/?key=${key}` } }
                }
            };
            const fcmResponse = await fetch(fcmUrl, { method: "POST", headers: { "Authorization": `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify(fcmPayload) });
...
async function getGoogleAuthToken(credentials) {
    ...
    const claim = { iss: credentials.client_email, scope: "https://www.googleapis.com/auth/firebase.messaging https://www.googleapis.com/auth/firebase.database", aud: "https://oauth2.googleapis.com/token", exp: now + 3600, iat: now };
